 ...
 */
export function createErrorResponse(message: string, type = "invalid_request_error", status = 400): Response {
  return new Response(
    JSON.stringify({
      error: {
        message,
 ...
 */
export function verifyAuth(req: Request, validKey?: string | null): boolean {
  if (!validKey || validKey === "1") {
    return true;
  }
